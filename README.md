...
const cntyCodeFields = [ 
				{ cntycd: 'USANCE_AFTER', cntynm: 'USANCE_FROM'}/* ,
...
const sections = [
        ...
        { id: 'charges', number: 2, title: 'Notes', tickId: 'tick2', numberId: 'number2', divid: '2_div' },
    ...
    $('div[id$="_div"]').each(function () {
			...
			rows.each(function () {
				...
				inputs.each(function (index) {
					...
					//Start Amount in foreign currency format 
						if ($input.hasClass('AMT_M') || $input.hasClass('AMT_P') || $input.hasClass('AMT_O')) {
							var currentAttr = $input.attr('onchange') || '';
							if (!currentAttr.includes('NumberConv')) {
								var NumberConv_Val = currentAttr.endsWith(';') ? ' NumberConv(event);' : '; NumberConv(event);';
								var updatedAttr = currentAttr ? currentAttr + NumberConv_Val : 'NumberConv(event);';
								$(this).attr('onchange', updatedAttr);
							}
							
						}
						//End Amount in foreign currency format 
				});
			});
...
wrapErrorRows();
    ...
    function renderTrxSysBtnsDiv1() {
        ...
        //$trxSysBtnsDiv.append($saveButton,$cancelButton, $nextButton, $prim_confirm);
