...
const sections = [
        ...
        { id: 'PartiesTab', number: 1, title: 'General Details', tickId: 'tick1', numberId: 'number1', divid: '1_div' },
        ...
        //{ id: 'Section_C_Tab', number: 4, title: 'Section C', tickId: 'tick4', numberId: 'number4', divid: '4_div' },
		{ id: 'undertakings', number: 3, title: 'Undertakings', tickId: 'tick3', numberId: 'number3', divid: '3_div' },
		{ id: 'DocumentsUpload', number: 4, title: 'Documents Upload', tickId: 'tick4', numberId: 'number4', divid: '4_div' },
		{ id: 'CommunicationTab', number: 5, title: 'Communication', tickId: 'tick5', numberId: 'number5', divid: '5_div' }
...
$(document).ready(function () {
    $('div[id$="_div"]').each(function () {
			...
			rows.each(function () {
                ...
                var buttons = $row.find('input[type="button"], button'); // Find input type=button or button inside the row
					...
					/* if ($row.hasClass('four-col-layout')) {
    ...
    for (let i = 0; i < cells.length; i += 2) {
        ...
        const $input = $inputCell.find('input, select, textarea');
...
if (isTwoColumnField) {
    ...
    if ($input.hasClass('DATE_M') || $input.hasClass('DATE_P') || $input.hasClass('DATE_O')) {
        ...
        var dateWrapper = $('<div class="date-wrapper" style="width:460px; height: 56px"></div>');
...
if (isTwoColumnField) {
    ...
    // === Amount + Currency dropdown fields ===
        ...
        amtWrapper.append(currencyDropdown).append($input);
        inputCell.append(amtWrapper);
    }else if ($input.hasClass('INT_M') || $input.hasClass('INT_O')|| $input.hasClass('INT_P') ) {
    $input.addClass('input-field');

    // Apply default styles for INT_M inputs
    $input.css({
        /* width: '100%',
        borderLeftWidth: '1px',
...
//TEST isFourColumnField
					var isFourColumnField = ($row.hasClass('four-field-inline-row') || $input.hasClass('four-field-inline-row'));
					var isTwoColumnFieldBottom = ($row.hasClass('two-col-align-bottom') || $input.hasClass('two-col-align-bottom'));
						console.log("isFourColumnField---->"+isFourColumnField);
						const headerRowFields = targetDiv.find('tr.four-field-inline-row');
//TEST isFourColumnField
					...
					else if ($input.length && $label.length && (!isFourColumnField)) {
...
//TEST isFourColumnField
					else if (headerRowFields.length >= 1) {
						const inlineRow = $('<div class="row-container four-heading-line"></div>');
					
						headerRowFields.each(function (index) {
							const $row = $(this);
							const labelText = $row.find('h3').text().trim();
					
							const fieldWrapper = $('<div class="inline-header-block" style="width:375px"></div>');
					
							if (index < 4) {
								// First 4 items as headings
								const labelElem = $('<h3 class="inline-header"></h3>').text(labelText);
								fieldWrapper.append(labelElem);
							} else {
								// Remaining items as input only (no label)
								const inputElem = $('<input type="text" class="inline-input" style="width:375px;height:35px;border-radius:10px;border-width: thin;margin-bottom:2px;padding-bottom:2px" placeholder="' + labelText + '" />');
								fieldWrapper.append(inputElem);
							}
					
							inlineRow.append(fieldWrapper);
							$row.remove(); // Remove original <tr> to avoid duplication
						});
					
						targetDiv.append(inlineRow);
					}
//TEST isFourColumnField
//TEST isTwoColumnFieldBottom
else if (isTwoColumnFieldBottom) { 
						var twoColRow = $('<div class="row-container two-column-row"></div>');
						var labelCell = $('<div class="label-cell"></div>').append($label);
						var inputCell = $('<div class="input-cell"></div>');
...
//TEST isTwoColumnFieldBottom
		...
		// Amount fields ,Add your Currency and amount fields respectively
			...
			const amountFields = [
				 ...
				 { ccy: 'EQUITY_FCCY', amt: 'EQUITY_AMT_FCY'},
				 { ccy: 'EQUITY_LCCY', amt: 'EQUITY_AMT_LCY'},
				 { ccy: 'LOAN_FCCY', amt: 'LOAN_AMT_FCY'},
				 { ccy: 'LOAN_LCCY', amt: 'LOAN_AMT_LCY'},
				 { ccy: 'NON_FUND_FCCY', amt: 'NON_FUND_AMT_FCY'},
				 { ccy: 'NON_FUND_LCCY', amt: 'NON_FUND_AMT_LCY'},
				 { ccy: 'TOTAL_FCCY', amt: 'TOTAL_AMT_FCY'},
				 { ccy: 'TOTAL_LCCY', amt: 'TOTAL_AMT_LCY'},
				 { ccy: 'PURPOSE_CODE', amt: 'PURPOSE_DESC'},
				/*{ ccy: 'PCFC_OTHER_CCY', amt: 'UNDER_AMT'} */
			
			];
				const cntyCodeFields = [
				
				{ cntycd: 'CNTY_CD_BENE_DRWR', cntynm: 'CNTY_NM_BENE_DRWR'}
				/*{ cntycd: 'PURPOSE_CODE', cntynm: 'PURPOSE_DESC'}*/
...
wrapErrorRows();
    ...
    function renderTrxSysBtnsDiv1() {
        ...
        var $cancelButton = $('<input>', {
            ...
            click: function () {
               // CancelTransaction();
			   getFuncIdOnClick_n('F05030702011')
            }
        });
        ...
        $trxSysBtnsDiv.append($saveButton, $createTemplateButton, $cancelButton, $nextButton, $prim_confirm);
